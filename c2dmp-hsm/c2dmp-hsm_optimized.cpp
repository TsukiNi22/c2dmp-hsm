...
Edition:
##  @date 15/03/2026 by @author Tsukini

File Name:
##  @file c2dmp-hsm_optimized.cpp
...
static consteval std::array<unsigned char, 256> make_lookup_table()
{
...
static constexpr inline float upper_limit_table[5][5] = {
    {2.0, 0.0,      0.0,        0.0,    0.0},
    {2.0, 1.0,      0.0,        0.0,    0.0},
    {2.0, 1.333333, 0.444444,   0.0,    0.0},
    {2.0, 1.5,      0.75,       0.1875, 0.0},
    {2.0, 1.6,      0.96,       0.384,  0.0768},
};
...
float c2dmp_optimized(const std::string_view a, const std::string_view b)
{
    ...
    coef = (coef < 1.01) ? 1.01 : coef;
    coef = (coef > 1.25) ? 1.25 : coef;
